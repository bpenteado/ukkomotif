...
def test_compute_kmer_frequencies():
    kmer_frequencies = compute_kmer_frequencies(sample_sequence_data, False, 3)
    ...
    with pytest.raises(FileNotFoundError):
        assert compute_kmer_frequencies(sample_sequence_data, True, 3)

    with open(full_sequence_file, "r") as file:
        full_sequence_data = file.read()
    
    kmer_frequencies = compute_kmer_frequencies(full_sequence_file, True, 3)
